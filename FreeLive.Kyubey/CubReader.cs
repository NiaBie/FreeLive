...
namespace FreeLive.Kyubey
{
    public class CubReader : L2BinaryReader
    {
       ...
       internal CubReader(Stream s) : base(s) { }
        ...
        public object ReadObject(int objType = -1)
        {
            ...
            switch (objType)
            {
                ...
                case (int)L2ObjType.Matrix2x3:
                    obj = new L2Matrix2x3F(ReadDouble(), ReadDouble(), ReadDouble(), ReadDouble(), ReadDouble(), ReadDouble());
                    break;
                ...
                default:
                    if (objType >= L2Constants.SERIALIZABLE_START)
                    {
